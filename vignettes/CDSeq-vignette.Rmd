...
```{r quick start, eval=TRUE, cache=FALSE, results='hide', class.source='klippy'} 
#################################################################
##                   set up CDSeq parameters                   ##
#################################################################
library(matrixStats)
# use the first 50 genes and first 10 synthetic mixtures as a quick example
gene_subset <- sort(rowVars(mixtureGEP), decreasing = TRUE, index.return=TRUE)$ix[1:50]
sample_subset <- 1:10
bulk_data <- mixtureGEP[gene_subset,sample_subset]     

# number of cell types in the mixtures
cell_type_number <- 6   

# number of iterations of MCMC procedure
mcmc_iter <- 700    

# number of CPUs that will be used for CDSeq. 
# This is used for parallel computing 
# when cell_type_number is a vector or reduce-recover is used.
cpu_number <- 1     

# effective gene length
gene_length <- as.vector(gene_length[gene_subset]) 

# pure cell line GEP, i.e. ground truth
ref_gep <- refGEP[gene_subset,]                    


#################################################################
##                          Run CDSeq                          ##
#################################################################
result1<-CDSeq(bulk_data =  bulk_data, 
               cell_type_number = cell_type_number, 
               mcmc_iterations = mcmc_iter, 
               gene_length = gene_length, 
               reference_gep = ref_gep, 
               cpu_number = cpu_number)
```

The output *result1* is a list containing multiple entities as listed in [Output](#output). When *cell_type_number* is a scalar, **CDSeq** will only returnt the following values.
```{r output, eval=TRUE, echo=TRUE}
ls(result1)
```
We can compare the CDSeq-estimated cell-type-specific GEPs with the ground truth.
```{r fig1, echo=TRUE,cache=FALSE,eval=TRUE}
trueGEP <- true_GEP_rpkm[gene_subset,result1$cell_type_assignment]
par(mar=c(4,4,1,1), mfrow=c(2,3), pty="s")
for (i in 1:6) {
    plot(result1$estGEP[,i],trueGEP[,i],xlab = "CDSeq-estimated GEPs", ylab = "True GEPs")
...
```{r fig2, echo=TRUE,cache=FALSE,eval=FALSE}
trueProp <- true_prop_cell[result1$cell_type_assignment,]
par(mar=c(4,4,1,1))
par(mfrow=c(2,3))
par(pty="s")
for (i in 1:6) {
    plot(result1$estProp[i,],trueProp[i,],xlab = "CDSeq-estimated proportions", ylab = "True proportions")
...
```{r example 2, eval=FALSE, cache=FALSE, results='hide'}
...
```{r example 4, eval=FALSE, cache=FALSE, results='hide'}
